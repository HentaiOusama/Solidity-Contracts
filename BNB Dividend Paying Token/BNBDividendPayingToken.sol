...
contract BNBDividendToken is ERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (
            isTradingEnabled && canSwap && !swapping &&
            !automatedMarketMakerPairs[from] && isSwappingEnabled &&
            from != owner() && to != owner()
        ) {
            swapping = true;
        ...
        if (takeFee) {
            ...
            __transfer(from, address(0), burnFees);
            __transfer(from, address(this), liquidityFees.add(holderFees));
        }
